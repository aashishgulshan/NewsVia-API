...
const News = ({ cat }) => {
  ...
  const fetchNews = async () => {
    await axios
      .get(
        cat
          ? `https://gnews.io/api/v4/top-headlines?category=general&lang=en&country=us&category=${cat}&max=10&apikey=a14009d402499c13aade4df186ed8c02
      `
          ...
          Data.map((items) => (
            <div className="shadow-lg rounded-lg hover:scale-105 flex-wrap bg-white">
              <div class="h-26 w-full overflow-hidden">
                <img
                  className="min-h-26 w-full rounded-t-lg overflow-hidden"
                  src={
                    !items.image
              ...
              <div class="text-sm text-gray-500 m-2">
                {items.publishedAt.substring(0, 10)}
              </div>
              <div class="font-bold text-lg leading-snug text-black m-2">
                {" "}
